...
class Puppet::Provider::Mongodb < Puppet::Provider
  ...
  def self.get_mongod_conf_file
    if @@configuration['server']
      configuration_file = "/etc/mongod_#{@@configuration['server']}.conf"
  ...
  def self.get_mongo_conf
    ...
    if config.kind_of?(Hash) # Using a valid YAML file for mongo 2.6
      config_hash['bindip']   = config.fetch('net', {}).fetch('bindIp', nil)
      config_hash['port']     = config.fetch('net', {}).fetch('port', nil)
      config_hash['ipv6']     = config.fetch('net', {}).fetch('ipv6', nil)
      config_hash['ssl']      = config.fetch('net', {}).fetch('ssl', {}).fetch('mode', nil)
      config_hash['sslcert']  = config.fetch('net', {}).fetch('ssl', {}).fetch('PEMKeyFile', nil)
      config_hash['sslca']    = config.fetch('net', {}).fetch('ssl', {}).fetch('CAFile', nil)
      config_hash['auth']     = config.fetch('security', {}).fetch('authorization', nil)
      config_hash['shardsvr'] = config.fetch('sharding', {}).fetch('clusterRole', nil)
      config_hash['confsvr']  = config.fetch('sharding', {}).fetch('clusterRole', nil)
  ...
  def self.is_admin_database_created
    return File.exists? @@configuration['created_file']
  end
  ...
  def self.mongo_cmd(db, host, cmd)
    ...
    if self.is_admin_database_created && self.auth_enabled
      args.push('--username', @@configuration['username']) if @@configuration['username']
      args.push('--password', @@configuration['password']) if @@configuration['password']
      args.push('--authenticationDatabase', @@configuration['db']) if @@configuration['db']
    end
